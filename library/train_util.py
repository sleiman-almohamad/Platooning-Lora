...
class BucketManager:
    ...
    def sort(self):
        # Sort by resolution (only for better appearance during display and metadata storage). Also rearrange buckets and reassign reso_to_id
    ...
    def set_predefined_resos(self, resos):
        # Store resolution and aspect ratio information when selecting from predefined sizes
    ...
    def select_bucket(self, image_width, image_height):
        ...
        if not self.no_upscale:
            ...
            # There might be the same aspect ratio (in fine tuning, when preprocessed with no_upscale=True), so prioritize those with the same resolution
            reso = (image_width, image_height)
            if reso in self.predefined_resos_set:
                pass
            else:
                ar_errors = self.predefined_aspect_ratios - aspect_ratio
                predefined_bucket_id = np.abs(ar_errors).argmin()  # The one with the smallest aspect ratio error other than the relevant resolution
            ...
            # Perform only reduction
            if image_width * image_height > self.max_area:
                # Image is too large, so determine bucket based on reduction while maintaining aspect ratio
                resized_width = math.sqrt(self.max_area * aspect_ratio)
                resized_height = self.max_area / resized_width
                assert abs(resized_width / resized_height - aspect_ratio) < 1e-2, "aspect is illegal"

                # Make the short or long side after resizing in reso_steps units: choose the one with smaller aspect ratio difference
            ...
            # Use image size as bucket size (crop without padding)
    ...
    def get_crop_ltrb(bucket_reso: Tuple[int, int], image_size: Tuple[int, int]):
        # Calculate crop left/top to match Stability AI preprocessing. Calculate crop right for flip augmentation
...
class AugHelper:
    # Removed dependency on albumentations but keep the same interface for now
...
class BaseSubset:
    def __init__(
        ...
        self.token_warmup_step = token_warmup_step  # Number of tags becomes maximum at N steps (if N<1, then N*max_train_steps)
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.current_epoch: int = 0  # Instance is created new for each epoch, so must be passed from outside
    ...
    def process_caption(self, subset: BaseSubset, caption):
        ...
        # Determine dropout: tag drop is in this method so it's good to do it here
    ...
    def new_cache_latents(self, model: Any, accelerator: Accelerator):
        r"""a brand new method to cache latents. This method caches latents with caching strategy.
...
# (original_size width, original_size height), (crop left, crop top)
...
class MinimalDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        r"""The subclass may have image_data for debug_dataset, which is a dict of ImageInfo objects.
...
# (original height),(crop left, crop top, crop right, crop bottom)
...
def load_images_and_masks_for_caching(
    ...
    r"""requires image_infos to have: [absolute_path or image], bucket_reso, resized_size
...
def cache_batch_latents(
    ...
    r"""requires image_infos to have: absolute_path, bucket_reso, resized_size, latents_npz
...
def verify_training_args(args: argparse.Namespace):
    r"""Verify training arguments. Also reflect highvram option to global variable
     highvram"""
